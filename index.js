...
async function startMzazi() {
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      const mzazi = require("./mahlon");
      mahlon(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(color("Congrats, 𝑴𝑨𝑯𝑳𝑶𝑵-𝑴𝑫 has successfully connected to this server", "green"));
      console.log(color("Follow me on Instagram as Nick_hunter9", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      client.sendMessage(client.user.id, { text: `> 𝗕𝗼𝘁 𝐢𝐬 𝗼𝗻𝗹𝗶𝗻𝗲【𝑴𝑨𝑯𝑳𝑶𝑵-𝑴𝑫】𝗶𝗳 𝘆𝗼𝘂 𝗻𝗲𝗲𝗱 𝗮𝗻𝘆 𝗵𝗲𝗹𝗽, 𝘁𝗲𝐱𝐭 𝗺𝗲 𝘁𝗵𝗿𝗼𝘂𝗴𝗵 +254737425382 𝗼𝗿 𝗮𝗻𝘆 𝗶𝘀𝘀𝘂𝗲` });
    }
  });
